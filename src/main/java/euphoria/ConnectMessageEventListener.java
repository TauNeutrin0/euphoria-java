...
public void onSendEvent(MessageEvent evt) {
  if(evt.getMessage().matches("^!sendbot @"+nick+" &[A-Za-z]+$")) {
    ...
    if (m.find()) {
      if(ConnectMessageEventListener.this.bot.isConnected(m.group(1))){
        if(ConnectMessageEventListener.this.hasDataFile) {
          ...
          if(!isStored) {
            ...
            try {
              ConnectMessageEventListener.this.dataFile.setJson(data);
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        RoomConnection c = ConnectMessageEventListener.this.bot.createRoomConnection(m.group(1));
        ...
        c.addConnectionEventListener(new ConnectionEventListener(){
            ...
            public void onConnect(ConnectionEvent arg0) {
              ...
              if(ConnectMessageEventListener.this.hasDataFile) {
                boolean isStored = false;
                for(int i=0;i<data.getAsJsonArray("rooms").size();i++){
                  if(data.getAsJsonArray("rooms").get(i).getAsString().equals(matcher.group(1))&&!matcher.group(1).equals("bots")){
                    isStored = true;
                  }
                }
                if(!isStored) {
        ...
        ConnectMessageEventListener.this.bot.startRoomConnection(c);
      }
    }
  }
    ...
    if (m.find()) {
      ...
      if(ConnectMessageEventListener.this.bot.isConnected(m.group(1))){
        ConnectMessageEventListener.this.bot.disconnectRoom(m.group(1));
        removed = true;
      }
